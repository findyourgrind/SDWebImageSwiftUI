...
public struct WebImage: View {
    private enum Placeholder {
        case image(Image)
        case builder(() -> AnyView)
    }
    @Binding var isAnimating: Bool
    let url: URL?
    let options: SDWebImageOptions
    let context: [SDWebImageContextOption: Any]?
    private var successBlock: ((PlatformImage, Data?, SDImageCacheType) -> Void)?
    private var progressBlock: ((Int, Int) -> Void)?
    private var failureBlock: ((Error) -> Void)?
    private var resizing: (EdgeInsets, Image.ResizingMode)?
    private var placeholder: Placeholder?
    
    public init(url: URL?, options: SDWebImageOptions = [], context: [SDWebImageContextOption : Any]? = nil, isAnimating: Binding<Bool> = .constant(false)) {
        self.url = url
    ...
    public var body: some View {
        if #available(iOS 14.0, *) {
            StateHolder(
                imageView: { configured(WebImageOriginal(imageManager: $0, isAnimating: $isAnimating)) },
                imageM: {
                    configure(imageManager: $0)
                    return $0
                })
        } else {
            configured(WebImageOriginal(url: url,
                                options: options,
                                context: context,
    ...
    private func configured(_ view: WebImageOriginal) -> some View {
        placeholdered(
            view
                .onSuccess(perform: successBlock)
                .onProgress(perform: progressBlock)
                .onFailure(perform: failureBlock)
                .resizable(resizing)
        )
    }
    
    @ViewBuilder
    private func placeholdered(_ view: WebImageOriginal) -> some View {
        switch placeholder {
        case let .image(image): view.placeholder(image)
        case let .builder(builder): view.placeholder(content: builder)
        case .none: view
        }
    }
    
    private func configure(imageManager: ImageManager) {
        imageManager.url = url
        imageManager.options = options
        imageManager.context = context
    }
}

private extension WebImageOriginal {
    func resizable(_ params: (EdgeInsets, Image.ResizingMode)?) -> WebImageOriginal {
        if let params = params {
            return resizable(capInsets: params.0, resizingMode: params.1)
        } else {
            return self
        }
    }
...
@available(iOS 14.0, *)
struct StateHolder<V: View>: View {
    @StateObject var imageManager: ImageManager = .init(url: nil)
    let imageView: (ImageManager) -> V
    let imageM: (ImageManager) -> ImageManager
    
    var body: some View {
        imageView(imageM(imageManager))
    }
}

extension WebImage {
    public func resizable(capInsets: EdgeInsets = .init(), resizingMode: Image.ResizingMode = .stretch) -> Self {
        var result = self
        result.resizing = (capInsets, resizingMode)
        return result
    }
    ...
    public func onSuccess(perform action: @escaping (PlatformImage) -> Void) -> WebImage {
        var result = self
        result.successBlock = { image, _, _ in
            action(image)
        }
        return result
    }
    ...
    public func onSuccess(perform action: @escaping (PlatformImage, SDImageCacheType) -> Void) -> WebImage {
        var result = self
        result.successBlock = { image, _, cacheType in
    ...
    public func placeholder<T>(@ViewBuilder content: () -> T) -> WebImage where T : View {
        var result = self
        let c = content()
        result.placeholder = .builder({ AnyView(c) })
        return result
    }
    ...
    public func placeholder(_ image: Image) -> WebImage {
        var result = self
        result.placeholder = .image(image)
        return result
    }
}
...
public struct WebImageOriginal : View {
    ...
    public init(imageManager: ImageManager, isAnimating: Binding<Bool>) {
    ...
    public var body: some View {
        ...
        return Group {
            if imageManager.image != nil {
                ...
                .onAppear {
                    imageManager.successBlock = successBlock
                    imageManager.progressBlock = progressBlock
                    imageManager.failureBlock = failureBlock
...
// Layout
@available(iOS 13.0, OSX 10.15, tvOS 13.0, watchOS 6.0, *)
extension WebImageOriginal {
    func configure(_ block: @escaping (Image) -> Image) -> WebImageOriginal {
        var result = self
    ...
    public func resizable(
        ...
        resizingMode: Image.ResizingMode = .stretch) -> WebImageOriginal
    {
    ...
    public func renderingMode(_ renderingMode: Image.TemplateRenderingMode?) -> WebImageOriginal {
        configure { $0.renderingMode(renderingMode) }
    }
    
    /// Configurate this view's image interpolation quality
    /// - Parameter interpolation: The interpolation quality
    public func interpolation(_ interpolation: Image.Interpolation) -> WebImageOriginal {
        configure { $0.interpolation(interpolation) }
    }
    
    /// Configurate this view's image antialiasing
    /// - Parameter isAntialiased: Whether or not to allow antialiasing
    public func antialiased(_ isAntialiased: Bool) -> WebImageOriginal {
...
extension WebImageOriginal {
    ...
    ///   - action: The action to perform. The first arg is the error during loading. If `action` is `nil`, the call has no effect.
    /// - Returns: A view that triggers `action` when this image load fails.
    public func onFailure(perform action: ((Error) -> Void)? = nil) -> WebImageOriginal {
        var result = self
        result.failureBlock = action
        return result
    }
    ...
    ///   - action: The action to perform. The first arg is the loaded image. If `action` is `nil`, the call has no effect.
    /// - Returns: A view that triggers `action` when this image load successes.
    public func onSuccess(perform action: @escaping (PlatformImage) -> Void) -> WebImageOriginal {
        var result = self
        result.successBlock = { image, _, _ in
            action(image)
        }
    ...
    ///   - action: The action to perform. The first arg is the loaded image, the second arg is the cache type loaded from. If `action` is `nil`, the call has no effect.
    /// - Returns: A view that triggers `action` when this image load successes.
    public func onSuccess(perform action: @escaping (PlatformImage, SDImageCacheType) -> Void) -> WebImageOriginal {
        var result = self
        result.successBlock = { image, _, cacheType in
            action(image, cacheType)
        }
    ...
    ///   - action: The action to perform. The first arg is the loaded image, the second arg is the loaded image data, the third arg is the cache type loaded from. If `action` is `nil`, the call has no effect.
    /// - Returns: A view that triggers `action` when this image load successes.
    public func onSuccess(perform action: ((PlatformImage, Data?, SDImageCacheType) -> Void)? = nil) -> WebImageOriginal {
        var result = self
        result.successBlock = action
        return result
    }
    ...
    ///   - action: The action to perform. The first arg is the received size, the second arg is the total size, all in bytes. If `action` is `nil`, the call has no effect.
    /// - Returns: A view that triggers `action` when this image load successes.
    public func onProgress(perform action: ((Int, Int) -> Void)? = nil) -> WebImageOriginal {
        var result = self
        result.progressBlock = action
        return result
    }
...
extension WebImageOriginal {
    ...
    /// - note: The differences between Placeholder and Indicator, is that placeholder does not supports animation, and return type is different
    /// - Parameter content: A view that describes the placeholder.
    public func placeholder<T>(@ViewBuilder content: () -> T) -> WebImageOriginal where T : View {
        var result = self
    ...
    /// - note: This placeholder image will apply the same size and resizable from WebImage for convenience. If you don't want this, use the ViewBuilder one above instead
    /// - Parameter image: A Image view that describes the placeholder.
    public func placeholder(_ image: Image) -> WebImageOriginal {
        return placeholder {
    ...
    public func retryOnAppear(_ flag: Bool) -> WebImageOriginal {
        var result = self
    ...
    public func cancelOnDisappear(_ flag: Bool) -> WebImageOriginal {
        var result = self
...
extension WebImageOriginal {
    ...
    public func customLoopCount(_ loopCount: UInt?) -> WebImageOriginal {
        var result = self
    ...
    public func maxBufferSize(_ bufferSize: UInt?) -> WebImageOriginal {
        var result = self
    ...
    public func runLoopMode(_ runLoopMode: RunLoop.Mode) -> WebImageOriginal {
        var result = self
    ...
    public func pausable(_ pausable: Bool) -> WebImageOriginal {
        var result = self
    ...
    public func purgeable(_ purgeable: Bool) -> WebImageOriginal {
        var result = self
    ...
    public func playbackRate(_ playbackRate: Double) -> WebImageOriginal {
        var result = self
...
struct WebImage_Previews : PreviewProvider {
    static var previews: some View {
        Group {
            WebImageOriginal(url: URL(string: "https://raw.githubusercontent.com/SDWebImage/SDWebImage/master/SDWebImage_logo.png"))
            .resizable()
